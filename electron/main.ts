...
const getDefaultWindowSize = () => {
  ...
  return {
    width: Math.max(
      Math.floor(width * WINDOW_CONFIG.WIDTH_SCALE),
      WINDOW_CONFIG.MIN_WIDTH
    ),
    height: Math.max(
      Math.floor(height * WINDOW_CONFIG.HEIGHT_SCALE),
...
const ensureNextJSServer = async (): Promise<number> => {
  if (nextJSServerInfo?.isRunning) {
    console.log(
      "Next.js server already running on port:",
      nextJSServerInfo.port
    );
  ...
  for (
    let attempt = 1;
    attempt <= NEXTJS_SERVER_CONFIG.RETRY_ATTEMPTS;
    attempt++
  ) {
    try {
      ...
      console.log(
        `Starting Next.js server on port: ${nextJSPort} (attempt ${attempt})`
      );
      ...
      console.error(
        `Error starting Next.js server (attempt ${attempt}):`,
        error
      );
      ...
      if (attempt === NEXTJS_SERVER_CONFIG.RETRY_ATTEMPTS) {
        ...
        throw new Error(
          `Failed to start Next.js server after ${NEXTJS_SERVER_CONFIG.RETRY_ATTEMPTS} attempts`
        );
      }

      await new Promise((resolve) =>
        setTimeout(resolve, NEXTJS_SERVER_CONFIG.RETRY_DELAY * attempt)
      );
    }
  }
...
const createWindow = async (): Promise<BrowserWindow> => {
  ...
  try {
    ...
    await mainWindow.loadURL(
      "data:text/html,<h1>Server Error</h1><p>Unable to start the Next.js server.</p>"
    );
  }
